	...
	switch ($this->params->get ( 'display_mode' )) {
		...
		case 0 :
			...
			for($j = 0; $j < count ( $this->pics ); $j ++) {
				echo JWallpapersHelperLayout::getCatThumbLayout ( $this->pics [$j], $this->cat_layout_params );
			}
			break;
		case 1 :
...
<?php
	...
	if ($this->params->get ( 'show_category_list' )) {
		...
		$countSubCats = count ( $this->mainCategories );
		echo '<h3>' . JText::_ ( 'CATEGORIES' ) . '</h3><ul>';
		
		for($i = 0; $i < count ( $this->mainCategories ); $i ++) {
			echo '<li><a href="' . $this->mainCategories [$i]->link . '">' . $this->mainCategories [$i]->title . '</a></li>';
		}
		?>
  </ul>
