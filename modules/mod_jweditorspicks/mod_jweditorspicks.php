<?php
/**
 * JWallpapers Editor's picks module - A module to display the latest tagged JWallpapers editor's picks pictures
 * 
 * @version 1.0 $Id: mod_jweditorspicks.php 353 2010-06-01 10:04:20Z amazeika $
 * @package JWallpapers
...
$items = $columns * $lines;
$query = $query = 'SELECT t1.id, t1.tag_ep, CASE WHEN CHAR_LENGTH(t1.item_deny_acl) THEN t1.item_deny_acl ELSE t3.item_deny_acl END AS item_deny_acl, CASE WHEN CHAR_LENGTH(t1.alias) THEN CONCAT_WS(\':\', t1.id, t1.alias) ELSE t1.id END AS slug, MONTH(upload_date) AS month, YEAR(upload_date) AS year, file_name AS name, file_ext AS ext, t1.title, t1.hits, average AS rating FROM #__jwallpapers_files AS t1 LEFT JOIN #__jwallpapers_votes_cache AS t2 ON t1.id = t2.file_id INNER JOIN #__jwallpapers_categories AS t3 ON t1.cat_id = t3.id WHERE t1.tag_ep = 1' . $where_cat_cond . $filter_restricted_pics_cond . ' AND t3.published = 1 AND t1.published = 1 ORDER BY tag_ep_date DESC';
...
if ($show_more_pics) {
	...
	$editors_picks_string = JFilterOutput::stringURLSafe ( JText::_ ( 'EDITORS_PICKS' ) );
	...
	if ($menu_id) {
		...
		$viewMoreLink = 'index.php?option=com_jwallpapers&view=special&id=5:' . $editors_picks_string;
	}
