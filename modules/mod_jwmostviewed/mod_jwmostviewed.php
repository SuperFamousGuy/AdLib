<?php
/**
 * JWallpapers Most Viewed module - A module to display the most viewed JWallpapers pictures
 * 
 * @version 1.1 $Id: mod_jwmostviewed.php 353 2010-06-01 10:04:20Z amazeika $
 * @package JWallpapers
...
$items = $columns * $lines;
$query = $query = 'SELECT t1.id, CASE WHEN CHAR_LENGTH(t1.item_deny_acl) THEN t1.item_deny_acl ELSE t3.item_deny_acl END AS item_deny_acl, tag_ep, CASE WHEN CHAR_LENGTH(t1.alias) THEN CONCAT_WS(\':\', t1.id, t1.alias) ELSE t1.id END AS slug, MONTH(upload_date) AS month, YEAR(upload_date) AS year, file_name AS name, file_ext AS ext, t1.title, t1.hits, average AS rating FROM #__jwallpapers_files AS t1 LEFT JOIN #__jwallpapers_votes_cache AS t2 ON t1.id = t2.file_id INNER JOIN #__jwallpapers_categories AS t3 ON t1.cat_id = t3.id WHERE t3.published = 1' . $where_cat_cond . $filter_restricted_pics_cond . ' AND t1.published = 1 ORDER BY hits DESC';
...
?>">
    ...
    <?php
				...
				foreach ( $rows as $row ) {
					...
					echo JWallpapersHelperLayout::getThumbLayout ( $row, $layout_params, $com_params );
					$i ++;
				}
				?>						
</ul>
...
if ($show_more_pics) {
	...
	if ($menu_id) {
		...
		$mostViewedString = JFilterOutput::stringURLSafe ( JText::_ ( 'MOST_VIEWED' ) );
		$viewMoreLink = 'index.php?option=com_jwallpapers&view=special&id=4:' . $mostViewedString;
	}
